...
const COMMENT_DONE_TEXT = "FOT Check Done";
...
async function main() {
  ...
  try {
    ...
    console.log("");
    console.log(`Found ${targets.length} applicant(s) to inspect for exact "${TRANSCRIPT_TITLE_EXACT}" materials.`);
    if (!options.save) {
      console.log("Dry run is enabled, so no Slate metadata will be changed.");
    }
    ...
    for (let index = 0; index < targets.length; index += 1) {
      ...
      try {
        const result = await syncApplicantFinalOfficialTranscripts({
          context,
          slatePage,
          slateSearchUrl,
        ...
        if (result.materials.length === 0) {
          console.log("  -> No exact Final Official Transcript materials found.");
          continue;
        }

        for (const material of result.materials) {
          const changeSummary = material.changes.length === 0
            ? "No changes needed."
            : material.changes.map((change) => `${change.label}: "${change.from}" -> "${change.to}"`).join(" | ");
          const modeLabel = options.save ? (material.saved ? "Saved" : "Not saved") : "Preview";
          console.log(`  -> ${material.schoolName} [${modeLabel}] ${changeSummary}`);
        }

        const transcriptCheckDone = target.rowNumber
          && sheetPage
          && sheetInfo
          && options.save
          && result.materials.length > 0
          && result.materials.every((material) => material.saved || material.changes.length === 0);

        if (transcriptCheckDone) {
          const wroteComment = await writeCommentsToSheet(sheetPage, sheetInfo, options, [
            {
              rowNumber: target.rowNumber,
              nNumber: target.nNumber,
              comment: COMMENT_DONE_TEXT,
            },
          ]);

          if (wroteComment.length === 0) {
            console.log(`[Write ${index + 1}/${targets.length}] Wrote row ${target.rowNumber} -> ${COMMENT_DONE_TEXT}`);
          } else {
            console.log(`[Write ${index + 1}/${targets.length}] Could not verify row ${target.rowNumber} -> ${COMMENT_DONE_TEXT}`);
          }
        }
      } catch (error) {
    ...
    console.log("");
    console.log(`Saved transcript-sync report to ${reportPath}`);
  } finally {
    terminal.close();
...
async function syncApplicantFinalOfficialTranscripts({ context, slatePage, slateSearchUrl, nNumber, save }) {
  ...
  const readInfo = await getReadApplicationInfo(slatePage);
  const readerPage = await context.newPage();

  try {
    const readerData = await loadReaderTranscriptData(readerPage, readInfo);
    const finalMaterials = readerData.finalMaterials;
    ...
    const streamText = readerData.streamText;
    const academicCredentials = readerData.academicCredentials;
    const materials = [];

    for (const material of finalMaterials) {
      const editContext = await fetchEditableContext(readerPage, material.materialId, readInfo.applicationId);
      const formId = await fetchFormIdForMaterial(readerPage, editContext.initialRecord, editContext.record, editContext.key);
      const form = await fetchMaterialForm(readerPage, editContext.initialRecord, formId, material.materialId, editContext.record);
      const transcriptText = streamText
        ? material.pageIndexes.map((pageIndex) => selectDataItemsToText(streamText[pageIndex]?.text ?? [])).join("\n\n")
        : await buildTranscriptTextFromRenderedPages(readerPage, readerData.transcriptStreamId, material.pageIndexes);
      const extracted = await extractTranscriptMetadata(readerPage, transcriptText, form, academicCredentials);
      const desired = buildDesiredFieldValues(form, extracted);
      const changes = diffFieldValues(form, desired);

      let saved = false;
      let verifiedFields = null;

      if (save && changes.length > 0) {
        await submitMaterialForm(readerPage, editContext, form, desired);
        await delay(1_500);
        const verifiedForm = await fetchMaterialForm(readerPage, editContext.initialRecord, formId, material.materialId, editContext.record);
        verifiedFields = verifiedForm.fields;
        saved = changes.every((change) => {
          const verified = verifiedForm.fieldMap.get(change.label);
          return normalizeSpace(verified?.displayValue ?? "") === normalizeSpace(change.to);
        });
      }

      materials.push({
        materialId: material.materialId,
        schoolName: desired.get("School Name *") ?? form.fieldMap.get("School Name *")?.displayValue ?? "Unknown school",
        transcriptTextPreview: transcriptText.slice(0, 1_200),
        extracted,
        changes,
        saved,
        verifiedFields,
...
function printHelp() {
  console.log(`Usage:
  npm run transcript:sync -- --n-number N18723936
  npm run transcript:sync -- --sheet-url "<google sheets url>" --sheet-tab Test
...
async function writeRunReport(results, options, sheetInfo) {
  ...
  const reportPath = path.join(reportsDir, `transcript-sync-report-${timestamp}.json`);
  const payload = {
...
function planRows(sheetRows, options) {
  ...
  rows = rows.filter((row) => normalizeSpace(row.originalComment) !== COMMENT_DONE_TEXT);
...
async function waitForSheetCommentValue(sheetPage, sheetInfo, options, rowNumber, expectedValue, timeoutMs) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    const currentValue = rows.find((row) => row.rowNumber === rowNumber)?.originalComment ?? "";
